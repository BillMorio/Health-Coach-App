...
export default function CoachLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems = [
    ...
    { name: 'Programs', href: '/coach/programs', icon: BookOpen },
    { name: 'Check-ins', href: '/coach/checkins', icon: CheckSquare },
    { name: 'Appointments', href: '/coach/appointments', icon: Calendar },
    ...
    <div className="flex min-h-screen bg-[#050505] text-zinc-100 font-sans">
      ...
      <aside className="w-64 border-r border-white/5 bg-white/[0.01] backdrop-blur-xl flex flex-col sticky top-0 h-screen z-20">
        <div className="p-8 border-b border-white/5 space-y-1">
          ...
          <p className="text-[10px] font-bold uppercase tracking-[0.3em] text-zinc-600 font-mono">Manager: {user?.name.split(' ')[0]}</p>
        </div>
        ...
        <header className="h-20 border-b border-white/5 bg-white/[0.01] backdrop-blur-md flex items-center justify-between px-10 sticky top-0 z-10">
          <div className="space-y-0.5">
            <h2 className="text-sm font-bold uppercase tracking-[0.2em] text-zinc-500 font-mono italic">
              Terminal_{navItems.find(i => i.href === pathname)?.name.replace(' ', '_').toUpperCase() || 'HUB_CENTER'}
            </h2>
            <p className="text-[10px] text-zinc-600 font-mono uppercase tracking-[0.3em]">Status: Professional</p>
          </div>

          <div className="flex items-center gap-4">
             <button className="btn-tactical !h-8 !px-3 flex items-center gap-2 group">
